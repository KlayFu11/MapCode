...
- `V1-F0-01` 至 `V1-F9-09` 已完成；v1 没有可执行的未完成任务。
- 若出现回归或验收证据与实现不一致，必须先按事实优先级创建明确的修复任务并通过对应门禁；不得把修复伪装成 v2，也不得直接开始下一产品版本。
- v2 仍未进入规划或开发。只有用户明确授权正式 v2 规划后，才能创建 v2 任务目录、进度账本或实现任务。
