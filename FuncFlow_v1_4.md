...
目录结构、代码文件、接口签名、测试策略和实现阶段查阅 `SPEC_v1_4.md`。
...
`SelectorModelRequest.system_prompt`、`user_prompt` 的精确文本与 `{max_selector_suggested_files}` 替换规则以 `SPEC_v1_4.md` 第 5.3 节为唯一权威；本 FuncFlow 只描述角色边界、数据来源和流转顺序，避免复制两份 prompt 后产生漂移。
