- # AGENTS.md
  ...
  - 与用户交流和执行任务时，默认从第一性原理出发：先确认要解决的真实问题、目标、成功标准和约束，而不是直接沿用用户给出的路径或已有惯例。
  - 不要默认用户已经完全清楚自己想要什么、为什么要做以及如何得到结果。若动机、目标、边界或成功标准不清晰，先停止实施，提出具体问题，围绕真实目标、可交付边界、风险和取舍与用户讨论。
  - 如果目标清晰但实现路径不是最短、最直接或最可验证，应明确指出，并给出更短路径、对应影响和保留原路径的代价。
  - 任务开始前把需求拆成“目标、边界、验证方式、最小可行路径”。实现过程中发现目标和路径不匹配时，优先回到目标澄清，不用沉没成本继续推进。
  - 对用户的方案保持审慎和批判，不把推测当事实；可以直接指出方案问题、隐藏假设和缺失证据，但必须给出可执行的下一步建议。
  ...
  16. 如果 `AGENTS.md` 已存在，不得运行 `/init` 生成新的规则入口文件。
  ...
  ### 8. Multi_Agent mode（显式启用）

  本模式只在用户明确要求“使用多 Agent 执行当前阶段施工”或明确指定 `Multi_Agent mode` 时启用。仅讨论、设计、审查或修改多 Agent 规则，不视为启用本模式。未启用时，原有单 Agent、普通子 Agent 和人工 commit gate 规则保持不变。

  #### 模式覆盖边界

  - 本模式只局部覆盖“主 Agent 每次只执行一个任务”和“必须等待用户逐任务批准后才 commit 并更新进度”两项默认规则。
  - 主 Agent 直接担任 coordinator，不得另外创建 coordinator Agent。
  - 每个会话只处理一个 `V1-F<n>` 阶段，按依赖顺序串行执行；正常目标为完成 2 个任务，最多 3 个。
  - 阶段已完成、门禁失败、出现阻塞或 context left 不足时，可以在少于 2 个任务时提前停止。

  #### 串行角色边界

  - **explorer**：只读任务文档、SPEC、PRD、FuncFlow 和相关源码，返回任务边界、文件计划、变量/类型、验证命令和风险。
  - **worker**：每次只实现一个任务，只能修改 explorer 计划授权的路径，不得修改项目进度，不得 commit。
  - **reviewer**：只审查 diff、执行验证并返回 `PASS` 或 `FAIL`，不得修改任何文件，不得 commit。
  - explorer、worker 和 reviewer 都不得调用 planning-with-files 创建执行文件；子 Agent 只返回对应任务的结构化结果，由主 Agent 写入任务运行目录。

  #### commit 与阶段审查

  - 只有 reviewer `PASS` 且主 Agent 最终 gate 通过后，主 Agent 才可以更新对应任务进度并 commit。
  - 每个任务只允许一个 commit；实现变更和 `doc/tasks/v1/progress.md` 对应勾选必须放入同一 commit，commit message 必须包含完整 `TASK_ID`。
  - 任一 gate 失败、测试失败、依赖不满足、SPEC 冲突、出现未预期 diff、路径越界或无法判断是否安全时，立即停止，不得 commit，不得启动下一任务。
  - 用户在本轮阶段批处理结束后对已完成的单任务 commit 做批量审查。主 Agent 不得自动 push。
