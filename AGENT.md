...
1. 文档中提到的SPEC、PRD等由于版本迭代的需要，在文件仓库中命名为SPEC_vx_x、PRD_vx_x等，如无明确说明，均默认调用最新版本的SPEC、PRD等文档，**落后版本的文档不要再去读**。
2. FuncFlow文档解释模块流程、数据走向、事件顺序和用户可见行为，是流程解释文档
3. 项目级事实必须写入 `doc/`。
4. 简单问题、单文件小修改、快速查询可以不创建 `.planning/` 目录，其他的符合`planning-with-files`skill描述的任务场景必须创建 `.planning/` 目录，禁止多个无关任务共用同一个 `.planning/` 目录。
5. 不允许只依赖聊天上下文传递需求、设计、任务状态、错误原因或测试结果。
6. 不允许在没有读取项目级文档的情况下直接实现代码。
7. 不允许把临时调研结论直接当成正式设计；调研结论必须经过确认后才能沉淀到 `doc/SPEC.md`。
8. 不允许为了通过测试而删除、跳过、注释掉失败逻辑；必须定位根因并记录处理过程。
9. 每次会话结束前必须更新相关文档，例如（.planning/<task>/task_plan.md -> 如任务拆分、依赖、验证命令变化，沉淀到 doc/tasks/{module-name}.md）确保下一个会话可以从文件恢复上下文。
10. 文档冲突时，采取 SPEC > PRD > FuncFlow > doc/tasks/*.md > doc/tasks/progress.md > .planning/<task>/task_plan.md > .planning/<task>/progress.md > .planning/<task>/findings.md的优先级
11. 如果 `.planning/` 或实现代码引入了 `doc/PRD.md` 中没有的行为，视为未批准范围。写入`.planning/<task>/findings.md`，不默认实现，需要时回到 `doc/PRD.md` 更新需求，需求确认后再实现。
12. 实现过程中发现`doc/SPEC.md`不合理写入`.planning/<task>/findings.md`，提出设计修正，更新`doc/SPEC.md`，再继续实现，禁止在不更新`doc/SPEC.md`的情况下偷偷加功能或者实现另一套设计。
13. 每个文档的结构模版参考各自对应的skill内容要求来布局。
14. 禁止在没有说明的情况下随意新增多个文档目录，除非已经明确规定这些目录的用途。
...
│   └── tasks/
│       ├── {module-name}.md          #模块任务说明
│       └── progress.md				 #项目级任务进度总账
├── .planning/
│   └── {YYYY-MM-DD}-{task-slug}/	  #会话时间以及会话执行的任务，例如2026-06-02-repo-intel-parser
...
2. 读取 doc/PRD.md
3. 读取 doc/SPEC.md
4. 读取 doc/tasks/progress.md
5. 读取对应 doc/tasks/{module-name}.md
6. 创建或继续 .planning/{YYYY-MM-DD}-{task-slug}/
7. 编写或更新 task_plan.md，没有task_plan.md不能开始该会话任务的实现
8. 执行代码阅读、调研、实现、测试
9. 持续更新 findings.md 和 progress.md
10. 任务完成后更新 doc/tasks/progress.md
11. 如有稳定设计变化，更新 doc/SPEC.md
12. 如有需求边界变化，更新 doc/PRD.md
13. 输出会话结束交付
...
持续更新 findings.md 和 progress.md
    ↓
任务完成后回写 doc/tasks/progress.md
    ↓
必要时回写 doc/SPEC.md、doc/PRD.md 或 doc/tasks/{module-name}.md
```
...
3. 任务粒度、依赖关系、输入输出、验证命令变化：更新 `doc/tasks/{module-name}.md`。
4. 任务完成状态变化：更新 `doc/tasks/progress.md`。
5. 当前会话计划变化：更新 `.planning/<task>/task_plan.md`。
6. 当前会话调研和代码阅读发现：更新 `.planning/<task>/findings.md`。
7. 当前会话执行日志、错误、测试结果：更新 `.planning/<task>/progress.md`。
...
3. 更新 `doc/tasks/<module-name>.md`。
4. 必要时更新 `doc/tasks/progress.md`。
5. 拆小任务后再继续。

#### 进度冲突

如果 `.planning/<task>/progress.md` 说任务完成，但 `doc/tasks/progress.md` 没有打勾，则任务在项目级仍未完成。
...
2. 检查测试。
3. 检查文档更新。
4. 确认无误后更新 `doc/tasks/progress.md`。

项目完成状态只以 `doc/tasks/progress.md` 为准。
