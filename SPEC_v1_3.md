...
- v1不将目录路径片段（如 `pico/`、`src/`）解析为 `mentioned_files`，也不引入 `mentioned_dirs`。目录片段仅保留在 original user_message 中，由 Branch B selector LLM 结合 broad repo map 与 SelectorCandidateCatalog 进行语义文件选择。selector 输出仍只通过 repo-relative file path 校验，不在 v1强制执行目录范围过滤。
...
| `MapSelector` helper | 构造 selector prompt、解析和校验结构化输出 | 不发起模型调用、不访问 MapEngine |
...
selector prompt、JSON DTO 和路径校验不属于 MapEngine，也不应堆进 Coordinator。
...
`build_selector_prompt()` 必须明确要求最多返回 `MAX_SELECTOR_SUGGESTED_FILES` 个文件。`parse_selector_output()` 在 JSON 成功解析后执行路径校验、稳定去重、建议数量限制和 reasoning 字符限制；不得先裁剪原始 JSON 再解析。
...
```text
specific = mentioned_files 非空 OR effective_symbol_hits 非空
fuzzy    = mentioned_files 为空 AND effective_symbol_hits 为空
```
...
- `RankingEvidence.personalization_files` 是 `focus_fnames` 中实际存在于本次文件图节点、并传给 PageRank personalization 的文件。该过滤在 `GraphRanker` 构图完成后、调用 PageRank/PPR 前执行，因此它始终是 `focus_fnames` 的有序子集。
...
`MapResult.mode` 表示控制流使用意图，不表示实际 ranking 算法。特别是 symbol-only Branch A 仍返回 `mode="focused"`，但由于 `focus_fnames=()`、`personalization_files=()`，其实际算法记录为 `algorithm="pagerank"`；若 PageRank 失败或图为空，则记录为 `algorithm="stable_path_fallback"`。

### 6.5 SelectorCandidateCatalog、SelectorResult 与 SelectionDecision

```python
@dataclass(frozen=True)
class SelectorCandidateCatalog:
    index_snapshot_id: str
    candidate_paths: tuple[str, ...]       # 全量稳定路径，仅用于输出校验
...
目录的全量校验集合与模型可见文本必须区分：

- `candidate_paths` 保存当前 snapshot 中全部已索引 Python 文件路径，按 repo-relative path 升序稳定排序，仅用于校验 selector 输出。
...
- selector 可以返回任意存在于 `candidate_paths` 的路径并通过校验，但模型通常只能主动发现 broad map 或 `rendered_text` 中实际可见的路径。不得把全量校验集合描述为模型已看到的全量目录。
...
- `selector_request_over_budget` 表示完整 selector prompt 超过 `ModelRequestBudget`，因此没有实际调用 selector，`selector_result=None` 且 `selector_model_calls` 不增加。
...
- `parse_selector_output()` 先成功解析完整 JSON，再规范化路径、按模型返回顺序稳定去重并校验。前 `MAX_SELECTOR_SUGGESTED_FILES` 个有效路径进入 `suggested_files`，其余有效路径进入 `excess_files`，无效路径进入 `invalid_files`；`reasoning` 最终最多保留 `SELECTOR_REASONING_MAX_CHARS` 个字符。
...
- `map_body_raw_chars` 是 MapEngine 候选 `{repo_map_text}` 的字符数，不包含固定导航安全契约、动态状态行或 fallback notice。
...
- `core/map_selector.py`：`SelectorResult`、`SelectionDecision` 和 `FallbackReason`。
...
- 从当前用户请求提取文件路径、basename、stem 和 identifier。
...
- 支持 repo-relative path 精确匹配。
- 支持唯一 basename 匹配。
- 支持唯一 path component / stem 匹配。
- basename、path component 或 stem 存在多个候选时，不写入 `mentioned_files`；v1 不新增交互式消歧流程。
- 所有输出必须是 repo-relative path。
- 输出按用户消息首次命中顺序稳定去重。
...
v1 可参考并移植 Aider 的启发式：

- mentioned identifier 边权重增强，匹配 ident 的引用边权重 ×10。
- personalization file 出站引用边增强，实际 PPR seed 文件引用外部 def 的边 ×50。
- 不得通过 Aider chat_fnames 参数路径实现 focus_fnames。
- 所有边权重修改必须发生在 PageRank 前。
- 高频引用平方根降权。
- 将实际存在于文件图节点的 `focus_fnames` 作为 PPR personalization seed，整体提升其 rank。
- PageRank / Personalized PageRank。

这三个机制作用层不同，不要混为一谈。symbol 命中（×10）和文件命中（personalization seed + ×50）对 ranking 的影响路径不同，这也是为什么文件命中通常比 symbol 命中更"精确"的原因。
...
- `focus_fnames` 保留调用方要求聚焦的文件；`personalization_files` 只保留其中实际存在于本次文件图节点的文件。
- `GraphRanker` 在图构建完成后、PageRank/PPR 前计算 `personalization_files`，保持 `focus_fnames` 输入顺序稳定去重。
...
2. 对 mentioned_idents 命中的引用边应用 ident_boost。
3. 从 focus_fnames 过滤得到实际存在于文件图节点的 personalization_files。
4. 对 personalization_files 的出站引用边应用 outbound_focus_boost。
5. 在完成所有边权重修改后调用 PageRank / Personalized PageRank。
6. 基于 PageRank 结果计算 Aider-style `(definer_file, identifier)` definition group rank，并生成 top_ranked_files 和 top_rank_contributors。
7. 按 definition group rank 排序现有 `DefinitionRecord`，并将该序列与文件级排序结果交给 ContextRenderer 做预算渲染。
8. focused generation 将 `effective_symbol_hits` 对应的 DefinitionRecord 固定置于渲染候选前缀；即使 symbol 没有引用边或 PageRank 分数较低，也必须优先进入 focused map。
...
identifier 命中 mentioned_idents 时：edge_weight *= IDENT_BOOST
source file 位于 personalization_files 时：edge_weight *= FOCUS_OUTBOUND_BOOST
多个 personalization_files 的 personalization 权重均匀分配
...
- top_rank_contributors：对该文件排名贡献最大的引用边，最多 top-3。
...
`generate_broad()` 使用标准 PageRank，不使用 personalization，不使用 focus outbound boost。
`generate_focused()` 始终返回 `mode="focused"`。它在 `personalization_files` 非空时使用 Personalized PageRank；当 `focus_fnames=()`，或 `focus_fnames` 均未形成文件图节点导致 `personalization_files=()` 时，使用标准 PageRank，但 mentioned_idents 的 ident boost 仍然生效。
...
| `generate_focused()` 且存在有效 seed | `focused` | 调用方输入 | 有效图节点子集 | `personalized_pagerank` |
| `generate_focused()` 且无有效 seed，包括 symbol-only Branch A | `focused` | 可能为空或非空 | `()` | `pagerank` |
...
if analysis.branch == "specific":
    ...
    if not self._can_confirm_focus():
        ...
        if self._selector_request_over_budget(selector_prompt):
            ...
            selector_raw = self._call_selector_model(selector_prompt)
            allowed_files = frozenset(selector_catalog.candidate_paths)
...
- `self._call_selector_model(selector_prompt) -> str` 是 Engine 内部适配点，必须复用 Pico 已有模型 provider/runtime 调用链；它不表示新增模型客户端或新的公共 runtime API。
...
- `selector_catalog` 与 broad map 复用同一个已就绪 SymbolIndex snapshot。selector 输入包含原始 user message、broad map 和预算受控的 `SelectorCandidateCatalog.rendered_text`，因此可以建议 broad map 中未出现、但在 catalog 可见目录里出现的文件。
- `allowed_files = frozenset(selector_catalog.candidate_paths)`。selector 只允许返回当前 SymbolIndex snapshot 中存在的 repo-relative path。
- 只有当完整 selector prompt 通过 `ModelRequestBudget` 检查后，Engine 才发送 `map_selector_requested`、递增 `selector_model_calls` 并真正发起 selector 调用。
...
- `build_selector_catalog()` 调用 `MapEngine.build_selector_catalog()`，返回同一 index snapshot 的全量校验路径集合和预算受控的候选目录文本，供 Engine 构造 selector prompt 和校验 selector 输出。
...
```text
mentioned_files 非空 OR effective_symbol_hits 非空
```
...
```text
mentioned_files 为空 AND effective_symbol_hits 为空
```
...
Coordinator.prepare_broad()
  ...
  -> Coordinator.build_selector_catalog() 生成同一 index snapshot 的全量校验路径集合和预算受控候选目录
  -> Engine 构造完整 selector prompt
  ...
  ->    Engine 使用 original prompt + broad map + SelectorCandidateCatalog 发起 selector 模型调用
  ->    MapSelector 校验输出
...
Pico 当前将 prefix、memory、history、repo map 和 current request 拼成单个主模型 prompt，没有为 MapCode 单独发送 provider-level System message。
...
- `Focus files to inspect first` 是读取优先级提示，不是已读取证明，也不是编辑授权。
...
- `{repo_map_text}` 只接受 `active_result.repo_map_text`。
...
| `map_prompt_analyzed` | PromptAnalysis 完成 | branch、files、idents、symbol hits |
| `map_context_ranked` | PageRank/PPR 完成 | algorithm、focus、top files、contributors |
...
- turn retrieval：branch、mode、focus/seeds、symbol hits、top ranked files、rendered/omitted、budget，以及在 artifact 已落盘后可用的 evidence path。
...
- basename / stem / path component 只有唯一候选时才进入 `mentioned_files`。
- Branch A 的 `focus_fnames` 只来自 `mentioned_files`；symbol-only Branch A 的 `focus_fnames=()`。
- symbol-only Branch A 返回 `MapResult.mode="focused"`，成功时 `personalization_files=()` 且 `RankingEvidence.algorithm="pagerank"`。
- `focus_fnames` 中未形成文件图节点的文件不会进入 `personalization_files`；当过滤后无有效 seed 时，focused generation 使用标准 PageRank。
- mentioned identifier 增权发生在 PageRank 前。
- personalization files 出站边增强发生在 PageRank 前。
...
- `analyze_turn()` 按 `ensure_index() -> map_index_status -> analyze() -> map_prompt_analyzed` 顺序执行，且一个 run 只发出一次 `map_index_status`。
...
- selector 输入包含 broad map 和预算受控的 SelectorCandidateCatalog.rendered_text。
- selector 只接受当前 SelectorCandidateCatalog 中存在的路径，包括未进入 broad map 的已索引文件。
...
| Step 7 | Engine Branch B selector + 整组二选一 confirm + fallback | 事件顺序、单选语义和调用统计通过 |
...
42. Branch B selector 使用当前 SymbolIndex snapshot 的全量 `candidate_paths` 和预算受控 `rendered_text`；可校验任意 `candidate_paths` 中的路径，但模型只显式看到 `rendered_text` 与 broad map 中出现的目录。
...
- selector prompt/parser 使用纯 helper。
