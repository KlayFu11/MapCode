...
- v1不将目录路径片段（如 `pico/`、`src/`）解析为 `mentioned_files`，也不引入 `mentioned_dirs`。目录片段仅保留在 `original_user_message` 中，由 Branch B selector LLM 结合 broad repo map 与 SelectorCandidateCatalog 进行语义文件选择。selector 输出仍只通过 repo-relative file path 校验，不在 v1强制执行目录范围过滤。
...
- selector system prompt 固定包含合法 JSON object 输出格式、可见文件限制、目录偏好软规则和渲染后的最大建议文件数。
