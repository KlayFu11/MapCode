...
MapCode 是基于 CodeDog runtime 的本地 Coding Agent Harness。v1 在 CodeDog 原有的模型调用、工具、审批、任务状态和运行审计链路上增加 `MapEngine`：它先为仓库生成可追溯的导航上下文（repo map），再由 CodeDog 主模型决定读取哪些完整源码。

Repo map 只回答“优先到哪里看”，不是源码替代品，也不授予工具额外权限。实际读取、编辑和 shell 操作仍由 CodeDog 的工具策略与审批链路控制。

## v1 能力边界

- 以 Git 仓库中的 Python 文件为输入，使用 definitions、references 和文件图生成稳定的 repo map。
- **Branch A**：请求中有明确文件、已索引 symbol 或 path ident 命中时，直接生成 focused map。`CodeDog/`、`src/` 这类 path ident 会形成 path personalization，但不会变成文件 focus，也不会调用 selector。
...
从 `CodeDog/` 目录启动，使用项目根目录的本地虚拟环境：

```bash
cd CodeDog
cp .env.example .env
# 在 .env 中填写 CodeDog_DEEPSEEK_API_KEY，或使用 DEEPSEEK_API_KEY。
set -a; source .env; set +a
PYTHONPATH=. ../.venv/bin/python -m CodeDog \
...
```bash
PYTHONPATH=. ../.venv/bin/python -m CodeDog --cwd /absolute/path/to/your/repository --map-engine --repl
PYTHONPATH=. ../.venv/bin/python -m CodeDog --cwd /absolute/path/to/your/repository --map-engine --tui
```
...
预算字段的优先级严格为：CLI 参数 > `.CodeDog.toml` 的 `[model_request_budget]` > 已选 provider profile > fallback。fallback 是 32,768 input tokens 与 1,024 safety-margin tokens；它是保守默认值，不能被当作某个模型的已验证窗口。
...
```bash
CodeDog --model-input-budget-tokens 98304 --prompt-safety-margin-tokens 2048
```
...
每次启用 MapEngine 的运行在工作区写入 `.CodeDog/runs/<run-id>/`。重点文件包括：
...
运行产物和 `.env` 都是本地数据，不应提交。固定 retrieval fixture、ground truth 和离线评测由 `CodeDog/tests/test_map_engine_retrieval_eval.py` 覆盖；完整的 v1 回归从 `CodeDog/` 目录运行：

```bash
PYTHONPATH=. ../.venv/bin/python -m pytest tests -q
../.venv/bin/python -m ruff check CodeDog tests
```
